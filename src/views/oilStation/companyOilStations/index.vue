  ...
  <div class="app-container calendar-list-container">
    ...
    <el-table
        ...
        <el-table-column align="center" label="打印机终端编号">
          <template slot-scope="scope" >
            <div v-if='scope.row.deviceId && scope.row.deviceId.length !== 0'>
              <div v-for="(item,index) in scope.row.deviceId" :key="index">{{ item }}
                <span v-if="index !== scope.row.deviceId.length - 1">,</span>
              </div>
            </div>
            <span v-else></span>
          </template>
