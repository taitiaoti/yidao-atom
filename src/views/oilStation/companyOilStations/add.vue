                  ...
                  <el-form label-width="100px" ref="form" :rules="rules" :model="formData">
                      <el-row :gutter="20">
                        <el-col :span="8">
                          ...
                          <el-form-item label="是否预存款" prop="preDeposit">
                              <el-select v-model="formData.preDeposit" placeholder="请选择">
                                  <el-option
                          ...
                          <el-form-item label="油 站 来 源" prop="source">
                          ...
                          <el-form-item label="收款渠道名称">
                              <el-select v-model="formData.paymentId" placeholder="请选择">
                                  <el-option
                          ...
                          <el-form-item label="加盟商ID" prop="gasUsername">
                              <el-input v-model="formData.gasUsername" placeholder="请输入加盟商ID"></el-input>
                          </el-form-item>
                          <el-form-item label="合伙人ID">
                              <el-input v-model="formData.parUsername" placeholder="请输入合伙人ID"></el-input>
                          </el-form-item>
                          <el-form-item label="关联打印机">
                              <div class="deviceIdBox" v-for="(item,index) in formData.deviceId" :key="index">
                                <el-input v-model="formData.deviceId[index]" placeholder="请输入终端编号" style="margin-right:10px;"></el-input>
                                <i :class="index == 0 ? 'el-icon-circle-plus-outline' : 'el-icon-remove-outline'" style="font-size:20px;" @click="addDeviceId(index)"></i>
                              </div>
                          </el-form-item>
                    ...
                    <div v-for="(header, index) in tabLabel" :key="index">
                      <el-table-column align="center" :label="header.label">
                        <template slot-scope="scope">
                          <el-input v-if=" header.prop == 'priceOfficial' || header.prop == 'priceGun' || header.prop == 'priceGas' || header.prop == 'priceYfq' " 
...
import { queryPayChannelList } from "@/api/franchisee/channel";
...
export default {
  data(){
    return {
      ...
      tabArr: [
        ...
        { label: '成本价（元）', prop: 'priceGas' },
        { label: '嘟嘟价（元）', prop: 'priceYfq' }
      ],
      tabLabel: [],
      tableList:[],
      productProperties: [],
      ...
      rules:{
        ...
        preDeposit: [
          { required: true, message: '请选择是否预存款', trigger: 'change' }
        ],
        source: [
          { required: true, message: '请选择油站来源', trigger: 'change' }
        ],
        gasType: [
          { required: true, message: '请选择油站品牌', trigger: 'change' }
        ],
        gasUsername: [
          { required: true, message: '请输入加盟商ID', trigger: 'blur' }
        ],
        // parUsername: [
        //   { required: true, message: '请输入合伙人ID', trigger: 'blur' }
        // ],
        // deviceId: [
  ...
  methods: {
    ...
    getChannelData(){
      ...
      console.log(this.channelData,'渠道')
    },
    // 获取油站详情可选项
    getDetailInfo(){
      let postData = {
    ...
    getGasInfo(gasId){
      ...
      queryGasStationInfo(postData).then((res)=>{
        ...
        if(gasInfo){
          ...
          this.formData.paymentId = gasInfo.paymentId;
          this.formData.deviceId = gasInfo.deviceId.length == 0 ? [''] : gasInfo.deviceId;
    ...
    addDeviceId(index){
      if(index == 0){
        // 添加
        this.formData.deviceId.push('');
      }else{
        // 删除
        this.formData.deviceId.splice(index,1);
      }
    },
    // 选择城市
    changeCity(valueArr){
