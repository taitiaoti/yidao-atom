    ...
    <div class="container">
        <div class="list-wrapper">
            ...
            <div class="list-item li02">
                <div>
                    <h2>活动类别</h2>
                    <div v-if="activityTypeData.length == 0" class="column-tip">暂无数据~</div>
                    <ul>
                        <li class="column" v-for="(item, index) in activityTypeData" 
                        :class="searchObj.activityTypeName == item.activityName ? 'cloumnName-sel' : ''" 
                        :key="index"
                        @click="handActivityType(item.activityName)">
            ...
            <div class="list-item list03">
                <div class="data-box" v-if="activityTypeData.length != 0">
        ...
        <el-dialog title="抢购商品信息" :visible.sync="dialogFormVisible" @close="handleCloseDialog" width="540px">
            <el-form :model="form" :rules="rules" ref="form">
                ...
                <el-form-item label="抢     购     价" :label-width="formLabelWidth" prop="price">
                ...
                <el-form-item label="最小购买数量" :label-width="formLabelWidth" prop="smallBuyNum">
                    <el-input v-model="form.smallBuyNum" autocomplete="off"></el-input>
                </el-form-item>
                <el-form-item label="最大购买数量" :label-width="formLabelWidth" prop="bigBuyNum">
                    <el-input v-model="form.bigBuyNum" autocomplete="off"></el-input>
                </el-form-item>
                <el-form-item label="需拼团满人数" :label-width="formLabelWidth" prop="groupNum">
                    <el-input v-model="form.groupNum" autocomplete="off"></el-input>
                </el-form-item>
                <el-form-item label="可中奖人数" :label-width="formLabelWidth" prop="winningNum">
                    <el-input v-model="form.winningNum" autocomplete="off"></el-input>
                </el-form-item>
                <el-form-item label="参与人奖金（每人/元）:" :label-width="formLabelWidth" prop="bonus">
...
<script>
    ...
    import { activityList, activityProductList, addActivityGoods, updateActivityGoods } from '@/api/groupPurchase/activitySetting'
    ...
    export default {
        data() {
            return {
                ...
                rules: {
                    ...
                    price: [{required: true, message: '请设置抢购价'}],
                    inventorys: [{required: true, message: '请设置总库存'}],
                    surplus: [{required: true, message: '请设置剩余库存'}],
                    smallBuyNum: [{required: true, message: '请设置最小购买数量'}],
                    bigBuyNum: [{required: true, message: '请设置最大购买数量'}],
                    groupNum: [{required: true, message: '请设置需拼满人数'}],
                    winningNum: [{required: true, message: '请设置可中将人数'}],
                    bonus: [{required: true, message: '请设置参与人奖金'}],
                },
                columnTotal: null
            }
        },
        ...
        methods: {
            ...
            // 选择活动类型
            handActivityType(name){
                this.searchObj.activityTypeName = name;
            ...
            getColumnList(){
                ...
                activityTypeList(postData).then((res)=>{
                  ...
                  if(res.data.records.length !== 0){
                      this.searchObj.activityTypeName = res.data.records[0].activityName;
                      this.getActivityData();
                  }
                })
            },
            ...
            getActivityData(){
              ...
              activityList(postData).then((res)=>{
                ...
                if(res.data.records.length !== 0){
                    this.activityId = res.data.records[0].id;
                    this.getGoodList();
                }else{
                    this.columnData = [];
                    this.total02 = 0;
            ...
            handleSubmit(formName){
                this.$refs[formName].validate(async (valid) => {
                    if(valid){
                        ...
                        addActivityGoods(this.form).then((res)=>{
                            console.log(res)
                            if(res.data.code == 0){
            ...
            handleEditor(row){
                ...
                this.form.bigBuyNum = row.bigBuyNum;
                this.form.bonus = row.bonus;
                this.form.groupNum = row.groupNum;
                this.form.id = row.id;
                this.form.inventorys = row.inventorys;
                this.form.placeholder = row.placeholder;
                this.form.price = row.price;
                this.form.productNo = row.productNo;
                this.form.smallBuyNum = row.smallBuyNum;
                this.form.winningNum = row.winningNum;
            ...
            async handleSubmitEditor(formName){
                this.$refs[formName].validate(async (valid) => {
                    if(valid){
                        ...
                        const res = await updateActivityGoods(this.form);
                        console.log(res,'编辑')
                        if(res.data.code == 0){
                            this.handleCloseDialog();
                            this.getGoodList();
            ...
            handleDelete(row){
                if(row.id){
                    ...
                    updateActivityGoods(postData).then((res)=>{
                        if(res.data.code == 0){
                            this.getGoodList();
                        }
                        this.notify(res);
