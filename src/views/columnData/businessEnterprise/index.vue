...
<script>
    import {
        fetchSamqiColumn,
        searchGoods,
        fetchSamqiColumnData,
        addDataForSamqiColumn,
        deleteDataForSamqiColumn,
        fetchEditorSamqyColumnData} from "@/api/columnData/businessEnterprise";
    ...
    export default {
        ...
        methods: {
            ...
            getColumnList(appId, appIdIndex){
                ...
                fetchSamqiColumn(postData).then(res => {
            ...
            getColumnData(id){
                ...
                fetchSamqiColumnData(id,postData).then(res => {
                    this.listLoading = false 
                    console.log(res, '商企栏目数据')
            ...
            add(){
                if(this.dataList.length != 0){
                    addDataForSamqiColumn(this.columnId, this.dataList).then(res => {
                        this.checkedItems = [];
                        this.dataList = [];
                        this.checkAll = false;
            ...
            handleDelete(row){
                if(row.id){
                    deleteDataForSamqiColumn(row.id).then(res => {
                        if(res.data.code == 0){
                            this.$notify({
                                title: res.statusText,
                                message: res.data.msg,
            ...
            editorColumnData(row){
                ...
                fetchEditorSamqyColumnData(postData).then(res => {
                    console.log(res, '修改');
                    //this.getColumnData(this.columnId);
                });
            },
