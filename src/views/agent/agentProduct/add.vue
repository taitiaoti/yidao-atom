  ...
  <div class="container">
    <el-tabs type="border-card" v-model="activeName" :before-leave="beforeLeave">
      <el-tab-pane label="基本信息" name="first">
        <span slot="label">
          <i class="icon-shangpin-jibenxinxi icon-14"></i> 基本信息
        </span>
        <el-form
        ...
        >
          <el-row :gutter="20">
            <el-col :span="8">
              ...
              <el-form-item label="所属分类" prop="currCategory">
                <el-cascader
                  :options="options"
                  :disabled="!isHidden(formData.appId || currentAppId)"
              ...
              <el-form-item label="所属栏目" prop="childColumnId">
                <el-select
                  v-model="formData.childColumnId"
                  placeholder="请选择所属栏目"
                  :disabled="!isHidden(formData.appId || this.currentAppId)"
                >
                  <el-option
              ...
              <el-form-item label="商品主图" prop="picture" ref="uploadElement">
                <el-upload
                  class="upload-demo"
                  :limit="1"
                  action="/shop/web/agentProduct/uploadMain"
              ...
              <!-- <el-form-item :label="'关  键  词'" prop="keywords">
                            <el-input v-model="formData.keywords" :disabled="!isHidden(formData.appId || currentAppId)"></el-input>
              </el-form-item>-->
              <!-- <el-form-item label="销量" prop="spu">
                            <el-input v-model="formData.spu" :disabled="!isHidden(formData.appId || currentAppId)"></el-input>
              </el-form-item>-->
              <!-- <el-form-item label="商品展位类型" prop="type">
              ...
              <!-- <el-form-item label="商品介绍类型" prop="descType">
              ...
              <el-form-item
                ...
                prop="costPrice"
              >
                <el-input placeholder="请输入供货价(元)" v-model="formData.costPrice" type="number"></el-input>
              </el-form-item>
              <!-- <el-form-item label="波动价" prop="wavePrice">
                              <el-input v-model="formData.wavePrice" type="number" :disabled="!isHidden(formData.appId || currentAppId)"></el-input>
              </el-form-item>-->
              <el-form-item label="建议零售价(元)" prop="unitPrice">
                <el-input
                  v-model="formData.unitPrice"
                  type="number"
                  placeholder="请输入建议零售价(元)"
                  :disabled="!isHidden(formData.appId || currentAppId)"
                ></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="8">
              ...
              <el-form-item label="对   比   来  源" prop="contrastSource">
                <el-select
                  v-model="formData.contrastSource"
                  @change="selectSource"
                  placeholder="请选择对比来源"
                  :disabled="!isHidden(formData.appId || this.currentAppId)"
                >
              ...
              <el-form-item label="对比零售价(元)" prop="contrastPrice">
                <el-input
                  v-model="formData.contrastPrice"
                  type="number"
                  placeholder="请输入对比零售价(元)"
                  :disabled="!isHidden(formData.appId || currentAppId)"
                ></el-input>
              </el-form-item>
              ...
              <el-form-item label="手   机   号  码" prop="phone">
                <el-input
                  ...
                  @keydown.native="inputLimitNumber"
                  placeholder="请输入联系人手机号码"
                  :disabled="!isHidden(formData.appId || currentAppId)"
                ></el-input>
              </el-form-item>
              <el-form-item label="所属 公司名称" prop="companyName">
                <el-input
              ...
              <el-form-item label="银 行 账 户 名" prop="acctName">
                <el-input
              ...
              <el-form-item label="银  行   卡  号" prop="bankAccount">
                <el-input
              ...
              <el-form-item label="所  属   银  行" prop="bankName">
                <el-input
              ...
              <el-form-item label="所  属   店  铺">
                <el-autocomplete
                  v-model="state"
                  :fetch-suggestions="querySearchAsync"
                  placeholder="请选择所属店铺名称"
                  @select="handleSelect"
                  style="width:100%"
                  :hide-loading= false
                  :disabled="!isHidden(formData.appId || currentAppId)"
              ...
              <!-- <el-form-item label="是否免邮">
                              <el-switch
                              :disabled="!isHidden(formData.appId || currentAppId)"
                              v-model="isFreePostage"
                              active-color="#409EFF"
                              inactive-color="#dcdfe6"
                              @change="onSwitch">
                              </el-switch>
              </el-form-item>-->
              <!-- <el-form-item label="邮费" v-if="!isFreePostage">
                              <el-input v-model="formData.postage" :disabled="!isHidden(formData.appId || currentAppId)"></el-input>
              </el-form-item>-->
            </el-col>
            <el-col :span="8">
      ...
      <!-- 商品介绍示例模板的弹出框 -->
      <el-dialog title="示例模板" :visible.sync="tupian4" width="41%" @click="tupian4 = false">
      ...
      <el-tab-pane label="商品介绍" name="second">
        <span slot="label">
          <i class="icon-shangpinjieshao icon-14"></i> 商品介绍
        </span>
        <div>
          <div style="float:left;color:red;">注意：</div>
          <div style="float:left;">
            <span>1.文字在1000字内</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            <span>2.图片最多上传10张，可包含营业执照和检测证书</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            <el-button @click="tupian4 = true" class="muban" size="mini">
              示例模板
        ...
        <QUE :text="defaultMsg" :editorId="editorId01" @editorChange="editorChange"></QUE>
      ...
      <!-- 商品图片的弹出框 -->
      <el-dialog title="示例模板" :visible.sync="tupian1" width="50%" @click="tupian1 = false">
      ...
      <el-tab-pane label="商品图片" name="third">
        <span slot="label">
          <i class="el-icon-picture icon-14"></i> 商品图片
        </span>
        <el-row>
          <el-upload
            class="upload-demo"
            action="/shop/web/agentProduct/uploadImages"
            :data="uploadArg"
            :on-success="uploadBannerSuccess"
            :on-change="handleBannerbefore"
            :file-list="tableData"
          ...
          >
            ...
            <div slot="tip" class="el-upload__tip">
              只能上传
              <span style="color:red;">jpg/png文件</span>，
              不超过
              <span style="color:red;">500kb</span>，
              尺寸大小为
              <span style="color:red;">1080x870</span>,
              且不能超过
              <span style="color:red;">5张</span>
            </div>
          </el-upload>
          <div>
            <el-button @click="tupian1 = true" class="muban" size="mini">
              示例模板
              <i class="el-icon-question el-icon--right"></i>
            </el-button>
          </div>
        ...
        <el-table :data="tableData" style="width: 100%">
          <el-table-column label="图片" width="200">
            <template scope="scope">
              <img
                :src="scope.row.url.indexOf('http')==0 ? scope.row.url : (baseUrl+scope.row.url)"
                height="100"
                width="100"
              >
            </template>
      ...
      <!-- 商品参数示例模板的弹出框 -->
      <el-dialog title="示例模板" :visible.sync="tupian2" width="45%" @click="tupian2 = false">
      ...
      <el-tab-pane label="商品参数" name="fourth">
        <span slot="label">
          <i class="icon-shangpincanshu icon-14"></i> 商品参数
        </span>
        <el-row :gutter="10" class="addParam">
          ...
          <el-col :span="1">
            <el-button
              ...
              type="primary"
              size="small"
              :disabled="!param || !paramValue"
              @click="addParam"
            >添加</el-button>
          </el-col>
          <el-col :span="2">
            <el-button @click="tupian2 = true" class="muban" size="mini">
              示例模板
              <i class="el-icon-question el-icon--right"></i>
            </el-button>
          </el-col>
        ...
        <el-table :data="paramTableData" style="width: 100%">
          <el-table-column prop="specName" label="参数名" width="300"></el-table-column>
          <el-table-column prop="specValue" label="参数值" width="300"></el-table-column>
          <el-table-column label="操作">
            <template scope="scope">
              <el-button
                @click="deleteRow(scope.$index, scope.row)"
        ...
        <!-- <el-row class="submit-good-desc">
                <el-button size="small" type="primary" @click="submitParam">确定</el-button>
        </el-row>-->
      </el-tab-pane>
      <!-- 商品规格示例模板的弹出框 -->
      <el-dialog title="示例模板" :visible.sync="tupian3" width="71%" @click="tupian3 = false">
      ...
      <el-tab-pane label="商品规格" name="five">
        <span slot="label">
          <i class="icon-shangpinguige icon-14"></i> 商品规格
        </span>
        <el-row class="p-table" :gutter="40">
          ...
          <el-col :span="2" style="text-align: right;margin-bottom: 20px;">
            <el-button @click="tupian3 = true" class="muban" size="mini">
              示例模板
              <i class="el-icon-question el-icon--right"></i>
            </el-button>
          </el-col>
        ...
        >
          <el-col :span="2" style="text-align: right;">
            <div style="position:relative;">
              <el-button
                ...
                style="padding:1px;position:absolute;top:2px;right:-8px;"
                size="mini"
                icon="el-icon-close"
                circle
              ></el-button>
              <el-button
                ...
                size="mini"
                @click="handleEditorProperty(item)"
          ...
          <el-col :span="22">
            <el-checkbox-group v-model="checkedSpecifition[index]" style="margin-top:10px;">
              <el-checkbox
                v-for="(item02, index02) in item.values"
                :disabled="!isHidden(formData.appId || currentAppId)"
                @change="checkInList"
                :label="item.id+','+item02.id"
                :key="index02"
              >{{item02.value}}</el-checkbox>
            </el-checkbox-group>
          </el-col>
        </el-row>
        <el-table
        ...
        >
          <template v-for="(header, index) in tableDataHeaderNew">
            <el-table-column :prop="header.prop" :label="header.label" :key="index">
              <template scope="scope">
                <el-input
                  :disabled="!(tableDataHeaderNew[index].prop == 'price') && !(tableDataHeaderNew[index].prop == 'isBenefit') && !(tableDataHeaderNew[index].prop == 'benefit') && !isHidden(appId)"
                  v-show="!(tableDataHeaderNew[index].change == '1') && !(tableDataHeaderNew[index].prop == 'isBenefit') && !(tableDataHeaderNew[index].prop == 'imageUrl')"
                  v-model="scope.row[tableDataHeaderNew[index].prop]"
                  placeholder
                ></el-input>
                <div
                  v-show="tableDataHeaderNew[index].change == '1'"
                >{{scope.row[tableDataHeaderNew[index].prop]}}</div>
                <el-select
                  :disabled="!(tableDataHeaderNew[index].prop == 'price') && !(tableDataHeaderNew[index].prop == 'isBenefit') && !(tableDataHeaderNew[index].prop == 'benefit') && !isHidden(appId)"
                  v-model="scope.row[tableDataHeaderNew[index].prop]"
                  v-show="tableDataHeaderNew[index].prop == 'isBenefit'"
                  placeholder="请选择"
                >
                  <el-option
                ...
                <el-upload
                  ...
                  v-show="tableDataHeaderNew[index].prop == 'imageUrl'"
                  :limit="1"
                  action="/shop/web/product/uploadInventoryImage"
                  :on-success="uploadPropertyPicSuccess"
                >
                  <el-button
                    size="small"
                    v-if="!scope.row[tableDataHeaderNew[index].prop]"
                    @click="getTableIndex(scope.$index)"
                    type="primary"
                    :disabled="!isHidden(formData.appId || currentAppId)"
                  >上传图片</el-button>
                </el-upload>
                <div
                  ...
                  v-show="tableDataHeaderNew[index].prop == 'imageUrl' && scope.row[tableDataHeaderNew[index].prop]"
                >
                  ...
                  <img
                    ...
                    :src="baseUrl+scope.row[tableDataHeaderNew[index].prop]"
                    height="80"
                    width="80"
                  >
                </div>
          ...
          <el-table-column label="操作" width="200">
            <template scope="scope">
              <el-button
                ...
                size="small"
                v-if="isHidden(appId)"
                @click="handelDeleteInventory(scope.$index, scope.row)"
              >删除</el-button>
            </template>
          </el-table-column>
        </el-table>
      ...
      <el-form :model="propertyForm" ref="form" label-width="100px">
        ...
        <div slot="footer" class="dialog-footer">
          ...
          <el-button
            ...
            @click="createForm('form')"
          >确 定</el-button>
          <el-button
            v-else
            type="primary"
            @click="updateProperty('form')"
            :loading="addPropertyLoading"
          >修 改</el-button>
        </div>
      </el-dialog>
      <el-tab-pane label="购物须知">
...
import { fetchAgentCas } from "@/api/product/category";
...
import { isvalidatemobile, inputLimitNumber } from "@/util/validate";
...
export default {
  ...
  data() {
    return {
      ...
      tupian4: false,
      //规格的弹出图片
      shangpin: "url(" + require("../../../assets/img/dudu/shangpin.png") + ")",
      activeName: "first",
      //参数的弹出图片
      canshu: "url(" + require("../../../assets/img/dudu/canshu.png") + ")",
      //规格的弹出图片
      guige: "url(" + require("../../../assets/img/dudu/guige.png") + ")",
      // //介绍的弹出图片
      jieshao: "url(" + require("../../../assets/img/dudu/jieshao.png") + ")",
      activeName: "first",
      id: "",
      editorId01: 1,
      editorId02: 2,
      ...
      purchaseNote: "",
      productNo: "",
      uploadArg: { productNo: this.productNo },
      goodSort: "其他",
      isFreePostage: true,
      formData: {
        productName: "",
      ...
      tableDataHeader: [],
      templ04: [
        { label: "零售价(元)", prop: "price" },
        { label: "对比零售价(元)", prop: "jdPrice" },
        { label: "库存(件)", prop: "number" },
        // {label:'已销售数量', prop: 'sku'},
      ...
      rules: {
        ...
        picture: [{ required: true, message: "请上传商品图片" }],
        keywords: [{ required: true, message: "请输入关键词" }],
        spu: [{ required: true, message: "请输入销量" }],
        type: [{ required: true, message: "请选择商品展位类型" }],
        descType: [{ required: true, message: "请选择商品介绍类型" }],
        weight: [{ required: true, message: "请输入重量(kg)" }],
        volume: [{ required: true, message: "请输入体积(m³)" }],
        costPrice: [{ required: true, message: "请输入供货价" }],
        wavePrice: [{ required: true, message: "请输入波动价" }],
        unitPrice: [{ required: true, message: "请输入零售价" }],
        contrastSource: [{ required: true, message: "请选择对比来源" }],
        childColumnId: [{ required: true, message: "请选择所属栏目" }],
        contrastPrice: [{ required: true, message: "请输入对比零售价" }],
        contacts: [{ required: true, message: "请输入联系人" }],
        phone: [{ required: true, validator: validPhone }],
        // currCategory: [{ required: true, message: '请选择所属分类',trigger: 'change',type:'array'}],
        companyName: [{ required: true, message: "请输入所属公司名称" }],
        bankName: [{ required: true, message: "请输入银行名称" }],
        acctName: [{ required: true, message: "请输入银行账户名" }],
  ...
  watch: {
    "formData.categoryId": function(newVal, oldVal) {
      //监听分类ID是否变化
      if (newVal && oldVal) {
        //新值newVal不为空
  ...
  methods: {
    // 搜索下拉框输入关键字请求店铺数据
     querySearchAsync(queryString, cb) {
        ...
        // if(queryString !== '' && queryString !== undefined){
          ...
          setTimeout(() => {
            // if(queryString.length > 0){
              var params = {appId:'dudu',storeName:queryString};
              getStoreName(params).then((resp)=>{
                console.log(resp,'所属店铺')
              if(resp.data == '' && queryString == ''){
                this.$notify({
                    title: '该用户没有注册店铺',
                    type: "error",
                    duration: 2000
      ...
      // 路由进来时根据id请求店铺信息
      getStoreInfoById(){
        console.log(this.formData,'店铺信息')
        this.restaurants = []
    ...
    inputLimitNumber: inputLimitNumber, //限制input框只能输入数字
    validateCarNumber() {
      let tempAccount = this.formData.bankAccount
        .replace(/\s/g, "")
        .replace(/[^\d]/g, "")
        .replace(/(\d{4})(?=\d)/g, "$1 ");
      this.$set(this.formData, "bankAccount", tempAccount);
    },
    getCategory() {
      fetchAgentCas(null).then(response => {
    ...
    getGoodInfo(id) {
      getObj(id).then(res => {
        ...
        res.data.unitPrice = this.saveDecimal(
          this.priceFilter(res.data.unitPrice || 0)
        );
        ...
        this.formData.isFreePostage == "n";
        // if(this.formData.isFreePostage == 'n'){
        //   this.isFreePostage = false
        // }else {
        //   this.isFreePostage = true
        // }
        this.fileList = [];
        this.fileList.push({
          name:
            this.formData.picture.indexOf("http") == 0
              ? this.formData.picture
              : this.baseUrl + this.formData.picture,
          url:
            this.formData.picture.indexOf("http") == 0
              ? this.formData.picture
              : this.baseUrl + this.formData.picture
    ...
    uploadSuccess(res, file) {
      ...
      this.rules.picture = [{ required: true, message: "请上传商品图片" }];
    ...
    handleChange(value) {
      ...
      getpropertyByCategoryId(categoryId, { productNo: this.productNo }).then(
        response => {
    ...
    handlePropertyByProductNo(inventories) {
      ...
      console.log(this.inventoryTableData, "this.inventoryTableData");
    ...
    submit(formName) {
      this.$refs[formName].validate(async valid => {
        if (valid) {
          // this.isFreePostage == false ? this.formData.isFreePostage = 'n' : this.formData.isFreePostage = 'y';
          this.formData.isFreePostage = "n";
          ...
          this.formData.descType = 0;
          console.log(this.idAddGood, "this.idAddGood");
          if (this.idAddGood) {
            if (!this.id) {
              console.log("添加");
    ...
    addOrPutObj(addOrPut, type) {
      ...
      addOrPut(this.formData).then(response => {
        ...
        this.$notify({
          title: response.data.data ? "成功" : "失败",
        ...
        if (!this.productNo && response.data.msg != "参数有误") {
          ...
          this.$notify({
            title: "添加成功",
            message: "请继续完善商品信息：商品介绍、商品图片、商品参数",
            type: "success",
            duration: 10000
          });
        }
    ...
    checkInList() {
      ...
      this.propertyList = [];
      for (let k in this.checkedSpecifition) {
        this.propertyList = this.propertyList.concat(
          this.checkedSpecifition[k]
        );
      }
          ...
          for (let k2 in this.inventoryTableData) {
            ...
            if (temp2.length > temp.length) {
              if (isContained(temp2, temp)) {
                ...
                this.inventoryTableData[k2].conversionPrice =
                  item.conversionPrice / 10000;
                this.inventoryTableData[k2].conversionBenefit =
                  item.conversionBenefit / 10000;
    ...
    delCheckedProperty(arr, row) {
      //删除规格规格数据时删除选中的规格
      for (var k in arr) {
        arr[k].forEach((item, index) => {
          if (
            item.split(",")[1] == JSON.parse(row.property)[item.split(",")[0]]
          ) {
    ...
    next(vm => {
      ...
      if (isDudu(vm.appId)) {
        ...
        if (vm.appIdList >= 2) {
          ...
          vm.currentAppId =
            vm.appIdList.length != 0 ? vm.appIdList[0].appId : "";
        }
      }
      vm.getCategory();
      if (vm.id) {
