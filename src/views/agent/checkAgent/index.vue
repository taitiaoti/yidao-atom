<template>
    <div class="app-container calendar-list-container">
        <div style="margin-bottom: 10px;">
            ...
            <el-input v-model="agentId" clearable style="width: 200px;" placeholder="请输入代理账号"></el-input>        
            <el-date-picker
                style="width: 400px;"
                v-model="searchTime"
                type="datetimerange"
                ...
                <el-row>
                    <el-col :span="12">
                        <div class="agent-dialog-item">
                            <span class="item-left">代理商账号：</span>
                            <span class="item-right">{{agentDetail.agentId}}</span>
                        </div>
                    </el-col>
                    <el-col :span="12"> 
                        <div class="agent-dialog-item">
                            <span class="item-left">代理商名称：</span>
                            <span class="item-right">{{agentDetail.agentName}}</span>
                        </div>
                    </el-col>
                </el-row>
                <el-row>
    ...
    export default{
        ...
        methods: {
            async getList(){
                ...
                const res = await checkAgentList(postData);
            ...
            async findAgent({id}){
                ...
                const res = await findAgentById(id);
            ...
            async submitRefuse(){
                const res = await agentRefuse(this.tempAgentId,{reason: this.reason});
                this.$notify({
                    title: res.data.data ? '成功' : '错误',
                    message: res.data.msg,
                    type: res.data.data ? 'success' : 'error',
