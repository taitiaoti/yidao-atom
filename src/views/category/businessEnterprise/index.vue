<template>
  <div class="app-container calendar-list-container">
    <div class="filter-container" style="display: flex;" v-if="business_enterprise_add">
...
<script>
  ...
  export default {
    ...
    created(){
      ...
      this.business_enterprise_editor = this.permissions["business_enterprise_editor"];
      this.business_enterprise_delete = this.permissions["business_enterprise_delete"];
      this.getList();
    },
    filters: {
    },
    methods:{
      ...
      getList(){
        ...
        fetchBusinessEnterprise(postData).then(res => {
      ...
      handleUpdate(row){
        ...
        this.id = row.id
        fetchBusinessEnterpriseById(row.id).then(res => {
      ...
      comfirmUpdate(){
        ...
        editorBusinessEnterprise(postData).then(res => {
            this.$notify({
      ...
      handleDelete(row){
        this.$alert('你确定删除该商企栏目？', '删除', {
          ...
          callback: action => {
            if(action == 'confirm'){
              if(row.id){
                deleteBusinessEnterprise(row.id).then(res => {
                  this.$notify({
                    title: res.statusText,
                    message: res.data.msg,
                    type: "success",
      ...
      updateOnShow(row){
        ...
        editorSamqiColumnOnshow(postData).then(res => {
          this.$notify({
            title: res.statusText,
            message: res.data.msg,
            type: "success",
