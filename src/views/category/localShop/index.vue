<template>
  <div class="app-container calendar-list-container">
    <div class="filter-container" style="display: flex;" v-if="store_column_add">
    ...
    <el-table :data="cartgoryList" v-loading="listLoading" element-loading-text="给我一点时间" border fit
      ...
      <el-table-column label="操作" align="center">
        <template slot-scope="scope">
          ...
          <el-button v-if="store_column_editor" size="small" type="success"
                     @click="handleUpdate(scope.row)">编辑
          </el-button>
          <el-button v-if="store_column_delete" size="mini" type="danger"
...
<script>
  import {
    fetchStoreColumn,
    fetchStoreColumnById,
    addStoreColumn,
    editorStoreColumn,
    deleteStoreColumn,
    editorStoreColumnOnshow
  } from "@/api/category/localShop";
  ...
  export default {
    data(){
      return {
        ...
        fileList: [],
        id: '',
        store_column_add:false,
        store_column_editor: false,
        store_column_delete: false,      
        listQuery: {
          page: 1,
          limit: 10
        },
    ...
    computed:{
      ...mapGetters(['permissions','storeColumnQueryList']),
      ...mapGetters('appId',{
        appIdList: 'appList'
      })
    },
    created(){
      ...
      this.listQuery.limit = this.storeColumnQueryList['limit'];
      this.listQuery.page = this.storeColumnQueryList['page'];
      this.store_column_add = this.permissions["store_column_add"];
      this.store_column_editor = this.permissions["store_column_editor"];
      this.store_column_delete = this.permissions["store_column_delete"];
      this.getList();
    },
    filters: {
    },
    methods:{
      ...mapMutations({
        setPage: 'set_storeColumn_page',
        setLimit: 'set_storeColumn_limit'
      }),
      getList(){
        ...
        fetchStoreColumn(postData).then(res => {
          console.log(res, '本地商家');
      ...
      handleUpdate(row){
        ...
        this.id = row.id
        fetchStoreColumnById(row.id).then(res => {
      ...
      comfirmUpdate(){
        ...
        editorStoreColumn(postData).then(res => {
            this.$notify({
      ...
      fecthAddColumn(){
        ...
        addStoreColumn(postData).then(res => {
          this.$notify({
            title: res.statusText,
            message: res.data.msg,
            type: "success",
      ...
      handleDelete(row){
        this.$alert('你确定删除该本地商家栏目？', '删除', {
          confirmButtonText: '确定',
          callback: action => {
            if(action == 'confirm'){
              if(row.id){
                deleteStoreColumn(row.id).then(res => {
                  this.$notify({
                    title: res.statusText,
                    message: res.data.msg,
                    type: "success",
      ...
      updateOnShow(row){
        ...
        editorStoreColumnOnshow(postData).then(res => {
          this.$notify({
            title: res.statusText,
            message: res.data.msg,
            type: "success",
