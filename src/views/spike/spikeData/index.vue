                ...
                <div class="data-box" v-if="columnList.length != 0">
                    ...
                    <div class="box-right">
                        ...
                        <el-table
                            ...
                            align="center"
                            width="200">
                            <template scope="scope">{{ scope.row.productNo }}</template>
                            </el-table-column>
                            <el-table-column
                            label="名称"
                            align="center"
                            >
                                <template scope="scope">{{ scope.row.productName }}</template>
                            </el-table-column>
...
<script>
    ...
    import { addActivityProduct, delActivityProduct, editorActivityProduct, getActivityProductById } from "@/api/spike/spikeData"
    import { fetchList } from "@/api/spike/spikeManage"
    ...
    export default {
        ...
        methods: {
            ...
            async handleSubmitEditor(formName){
                this.$refs[formName].validate(async (valid) => {
                    if(valid){
                        const res = await editorActivityProduct(this.form);
                        this.$notify({
            ...
            getColumnData(id){
                ...
                getActivityProductById(id,postData).then(res => {
            ...
            add(){
                if(this.dataList.length != 0){
                    addActivityProduct(this.columnId, this.dataList).then(res => {
                        this.checkedItems = [];
                        this.dataList = [];
                        this.checkAll = false;
            ...
            handleDelete(row){
                if(row.id){
                    delActivityProduct(row.id).then(res => {
                        if(res.data.code == 0){
                            this.$notify({
                                title: res.data.data ? '成功' : '失败',
                                message: res.data.msg,
