<template>
    <div class="app-container calendar-list-container">
        <div class="filter-container">
            ...
            <el-input v-model="searchObj.orderNo" @change="onOrderNo" style="width: 200px;margin-bottom: 10px;" prefix-icon="el-icon-search" placeholder="请输入订单编号" clearable></el-input>
            <!-- <el-input v-model="searchObj.uid" @change="onUid" style="width: 200px;margin-bottom: 10px;" prefix-icon="el-icon-search" placeholder="请手机号码"></el-input> -->
            <div class="s-category" style="width:360px;display:inline-block;margin-bottom:10px;">
        ...
        <el-table :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit
            ...
            <el-table-column align="center" label="购买数量(件)" width="120">
                <template slot-scope="scope">
                    <span>{{ scope.row.productNum }}</span>
                </template>
            </el-table-column>
            <el-table-column align="center" label="订单金额(元)" width="130">
                <template slot-scope="scope">
            ...
            <!-- <el-table-column align="center" label="京东订单号" width="130">
                <template slot-scope="scope">
            ...
            <el-table-column v-if="agent_order_look || agent_order_returnGoods" align="center" label="操作" width="180">
                <template slot-scope="scope">
                    <el-button size="small" :disabled="scope.row.vender != username" v-if="agent_order_look" type="primary"
                               @click="jumpEditor(scope.row)">
                               查看
                    </el-button>
                    <el-button :disabled="scope.row.vender != username" v-if="agent_order_returnGoods" size="small" type="danger"
...
<script>
    import { fetchAgentList,fetchShipper,fetchExpressInfo,addExpressInfo, fetchOrderAgentExport,fetchReturnGoods } from '@/api/order/index'
    ...
    let websiteBaseUrl = config.dev.proxyTable['/admin'].target;
    export default {
        name: 'table_t_order',
        directives: {
            waves
        ...
        data() {
            return {
                ...
                agent_order_look: false,
                agent_order_returnGoods: false,
                exportLoading: false,
                isProAgent: '',
                agentProList: [],
                username: ''
        ...
        created() {
            ...
            console.log(this.orderSearch,'orderSearch')
            this.getList()
            // this.search();
            this.agent_order_look = this.permissions['agent_order_look']
            this.agent_order_returnGoods = this.permissions['agent_order_returnGoods']
        },
        methods: {
            ...mapMutations({
            ...
            async getList() {
                ...
                fetchAgentList(postData).then(response => {
            ...
            async orderExport(){
                ...
                // const res = await fetchOrderAgentExport(postData);
                // console.log(res,'ddd');
                
                window.location.href = websiteBaseUrl+'/shop/web/order/agentExport?orderStatus='+postData.orderStatus+'&orderNo='+postData.orderNo+'&phone='+postData.phone+'&appId='+postData.appId+'&startTime='+postData.startTime+'&endTime='+postData.endTime+'&vender='+this.userInfo.username
