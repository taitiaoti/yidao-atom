    ...
    <div class="app-container calendar-list-container">
        ...
        <el-table :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit
            ...
            <el-table-column align="center" label="订单状态" width="100">
                <template slot-scope="scope">
                    <span v-if=" scope.row.refundStatus == '1' || scope.row.refundStatus == '3' ">买家申请退款</span>
                    <span v-if=" scope.row.refundStatus == '2' ">已退款</span>
                    <span v-if=" !scope.row.refundStatus ">{{ scope.row.orderStatus | orderStatusFilter}}</span>
                </template>
            </el-table-column>
            <el-table-column align="center" label="卖家是否同意退款" width="100">
                <template slot-scope="scope">
                    <span>{{ scope.row.merchantsStatus | filterMerchantsStatus }}</span>
                </template>
            </el-table-column>merchantsStatus
...
<script>
    ...
    export default {
        ...
        computed: {
            ...mapGetters(['orderSearch','permissions','sanctionState']),
            ...mapGetters('appId',{
                appIdList: 'appList'
            })
        },
