    ...
    export default {
        ...
        created(){
            if(this.$route.query.orderInfo){
                this.payUrl = this.$route.query.orderInfo.data;
                this.orderNo = this.$route.query.orderInfo.orderNo;
                this.payCode = this.$route.query.orderInfo.payCode;
                this.source = this.$route.query.orderInfo.source;
                this.getPayStatus();
            }
        },
        methods: {
            getPayStatus(){
                this.timer = setInterval(async () => {
                    ...
                    if(res.data.code == 0){
                        ...
                        this.$store.dispatch('cartJd/checkout', this.cartSelectItems); //清空购物车里已购买的商品
                        this.isJump = true;
                        this.$notify({
                            title: '付款成功',
                            message: '完成本次下单',
