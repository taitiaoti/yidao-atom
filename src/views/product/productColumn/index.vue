<template>
  <div class="app-container calendar-list-container">
    <div class="filter-container" style="display: flex;" v-if="product_column_add">
    ...
    <el-table :data="cartgoryList" v-loading="listLoading" element-loading-text="给我一点时间" border fit
      ...
      <el-table-column label="操作" align="center">
        <template scope="scope">
          ...
          <el-button v-if="product_column_editor" size="small" type="success"
                     @click="handleUpdate(scope.row)">编辑
          </el-button>
          <el-button v-if="product_column_delete" size="mini" type="danger"
...
<script>
  import { fetchList, fetchSingleListById, fetchUpdateList,fetchAdd,fetchShow,fetchDel } from "@/api/product/productColumn";
  import {mapGetters, mapMutations} from "vuex";
  let appList = [];
  if(localStorage.getItem('currentApp')) { appList = JSON.parse(localStorage.getItem('currentApp'));}
  ...
  export default {
    data(){
      return {
        ...
        fileList: [],
        id: '',
        product_column_add:false,
        product_column_editor: false,
        product_column_delete: false,      
        listQuery: {
          page: 1,
          limit: 10
        },
    ...
    created(){
      ...
      this.product_column_add = this.permissions["product_column_add"];
      this.product_column_editor = this.permissions["product_column_editor"];
      this.product_column_delete = this.permissions["product_column_delete"];
      this.getList();
    },
    filters: {
    },
    methods:{
      ...
      handleUpdate(row){
        ...
        this.id = row.id
        fetchSingleListById(row.id).then(res => {
      ...
      comfirmUpdate(){
        ...
        fetchUpdateList(postData).then(res => {
            this.$notify({
