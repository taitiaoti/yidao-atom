    ...
    >
      <el-form-item label="收货人信息" class="default-address-wrapper">
        ...
        <div class="default-address">
          <el-tag>默认地址</el-tag>
          <span class="text">
            {{defaultAddressObj.name}}&nbsp;&nbsp;&nbsp;&nbsp;{{defaultAddressObj.province}} {{defaultAddressObj.city}}
      ...
      <el-form-item label="商品清单">
        ...
        >
          <el-table-column width="100">
            <template scope="scope">
              <img
                :src="scope.row.picture.indexOf('http') != -1 ? scope.row.picture : (baseUrl+scope.row.picture)"
                width="80"
                height="80"
                alt
              >
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="3" class="price-wrapper">
            <span class="price">-￥{{ productMoney - discountAmount + Number(freight) }}</span>
          </el-col>
        </el-row>
        <el-row :gutter="20">
          <el-col :span="21" class="num-wrapper">
        ...
        <el-row :gutter="20" class="s-address-wrapper">
          <el-col :span="24">
            <span class="address">
              寄送至：{{defaultAddressObj.province}} {{defaultAddressObj.city}} {{defaultAddressObj.area}} {{defaultAddressObj.town}}
        ...
        <el-row :gutter="20">
          <el-col :span="24" class="submit-order">
            ...
            <el-button
              type="primary"
              :disabled="cartSelectItems.length == 0 || !isSubmit"
              :loading="loading"
              @click="submitOrder"
            >提交订单</el-button>
          </el-col>
    ...
    <el-dialog title="收货地址" :visible.sync="dialogFormVisible">
      <el-form :model="addressData">
        ...
        <el-form-item label="默认地址" :label-width="formLabelWidth">
          <el-switch v-model="addressData.isDefault" @change="changeDefault"></el-switch>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    async getDealCartPrice(id) {
      //处理购物车价格
      this.isSubmit = true;
        // var x = new Set(this.cartSelectItems.property);
        // console.log(x,789)
        // console.log(this.cartSelectItems,789)
      this.cartSelectItems.forEach(item => {
        // var x = new Set()
      
        // return [...x];
        // console.log(item.propertytem,456);
      ...
      // console.log(this.paramsProducts, 123456);
      //直接截取数组最后的一个下标
      // console.log()
      // let ceshiData = [];
      // ceshiData.push(this.paramsProducts[this.paramsProducts.length - 1]);

      // console.log(ceshiData, "测试数据");
      // console.log(this.paramsProducts, "侧");
      let postData = {
        products: this.paramsProducts,
        // products: ceshiData,
        deliveryId: id
      };
      console.log(postData,'测试数据')
    ...
    getSkusAndNum() {
      ...
      console.log(this.cartSelectItems, 22222);
      return tempArr;
      console.log(tempArr, 123456);
    },
    ...
    async submitOrder() {
      let ceshiData2 = [];
      ceshiData2.push(this.paramsProducts[this.paramsProducts.length - 1]);
      let postData = {
      ...
      if (!this.defaultAddressObj.id) {
        ...
        if (this.payChannel == "redpay") {
          ...
          if (res.data.code === 0) {
            this.$store.dispatch("cartBusiness/checkout", this.cartSelectItems);
            this.$router.push({
          ...
          if (res.data.code === 0) {
            this.$store.dispatch("cartBusiness/setOrderInfo", res.data.data);
            this.$router.push({
