    ...
    <div class="purchase-business" v-loading="listLoading" element-loading-text="给我一点时间">
        ...
        <el-row>
            <el-col :span="4" v-for="(item, index) in list" :key="index">
                <div  class="grid-content jd-good" :class="[goodIndex == index ? 'jd-good-sel' : '']" @mouseenter="enter(index)" @mouseleave="leave(index)">
                    ...
                    <div class="price-wrapper">
                        <span class="price-jd"> 
                            ...
                            <span v-if="item.contrastSource==7" >京东价:</span> 
                ...
                <div class="info-right" v-if="property.length != 0">
                    <div class="right-property">
                        ...
                        <el-radio-group v-model="deliveryType" size="mini">
                            <el-radio v-for="(item, index) in JSON.parse(this.goodDetail.deliveryMode)" @change="selectDeliveryType(product,deliveryType)" :label="item.code"  :key="index" border>{{item.name}}</el-radio>
                        </el-radio-group>
                    </div>
                    <div class="right-property" v-for="(item, index) in property" :key="index">
                        ...
                        <el-radio-group v-model="values[index]" size="mini">
                            <el-radio @change="selectProperty({propertyNameId:item.propertyNameId, propertyValueId:item02.propertyValueId, item:product, deliveryType:deliveryType })" v-for="(item02, index02) in item.propertyValues" :label="item02.propertyValueId" :key="index02" border>{{item02.propertyValueName}}</el-radio>
                        </el-radio-group>
                    </div>
                    <div class="right-property">
                        <div class="property-title">购买数量</div>
                        <el-input-number :disabled="!selectInventory" v-model="quantity" :min="deliveryType === 'express' ? 0 : logisticsMinNum" :step="deliveryType === 'express' ? 1 : addNum"  @change="handleChange(product,deliveryType)" size="small" style="width: 120px;"></el-input-number>
...
<script>
    ...
    import { getproducts, getProperty, getGoodDetail } from '@/api/product/purchaseBusiness'
    ...
    export default {
        ...
        methods: {
            ...
            async add(item){
                ...
                const resArr = await Promise.all([getProperty(this.product.productNo), await getGoodDetail(item.productNo)]);
