...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            handleChange(item){
                this.tempItem = item;
                // item.property = this.selectInventory; //给添加进购物车的商城添加属性
                // item.quantity = this.quantity;  //给添加进购物车的商城添加数量
                // item.handleType = this.handleType //操作类型减少 或者 添加
                // this.$store.dispatch('cartBusiness/addProductToCart',item);
            },
            jumpCart(){
                this.$router.push({
