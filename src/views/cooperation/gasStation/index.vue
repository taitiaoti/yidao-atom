    ...
    <div class="filter-container">
      ...
      <el-table
        ...
        <el-table-column align="center" label="油站名称">
          <template scope="scope">
            <span>{{scope.row.gasStationName}}</span>
          </template>
        </el-table-column>
        <el-table-column align="center" label="油站数量">
          <template scope="scope">
            <span>{{scope.row.gasStationNumber}}</span>
          </template>
        </el-table-column>
        <el-table-column align="center" label="油站地址">
          <template scope="scope">
            <span>{{scope.row.gasStationAddress}}</span>
          </template>
        </el-table-column>
...
import { gasJoining,exportGasPromoters } from '@/api/franchisee/index';
export default {
    data(){
        return {
            searchObj: {
    ...
    methods: {
        ...
        getList(){
            ...
            gasJoining(postData).then((res)=>{
                console.log(res,'加油站加盟列表')
        ...
        handleExport(){
          ...
          exportGasPromoters(postData).then((res)=>{
              // console.log(res,'导出')
              this.loading = false;
              if(res.status == 200){
