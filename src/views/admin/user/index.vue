<template>
  <div class="app-container calendar-list-container">
    <div class="filter-container">
      <el-input @keyup.enter.native="search" style="width: 200px;" class="filter-item" placeholder="用户名" v-model="listQuery.username">
      </el-input>
    ...
    <el-table :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row style="width: 99%">
      ...
      <el-table-column align="center" label="优惠券总额">
        <template slot-scope="scope">
          <span>{{scope.row.coupon | filterMoney}}</span>
        </template>
      </el-table-column>
      ...
      <!-- <el-table-column align="center" label="代理商名称">
        <template slot-scope="scope">
          <span v-for="(role, index) in scope.row.roleList" :key="index">{{role.agentName}} </span>
        </template>
      </el-table-column> -->
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form :model="form" :rules="rules" ref="form" label-width="100px">
        ...
        <el-form-item label="密码" placeholder="请输入密码" prop="password">
          <el-input type="password" :disabled="dialogStatus != 'create'" v-model="form.password"></el-input>
        </el-form-item>
        ...
        <el-form-item label="角色" prop="role">
          <el-select class="filter-item" v-model="form.role" @change="handleChangeTage" filterable allow-create placeholder="请选择" multiple>
        ...
        <el-form-item label="优惠券总额">
          <el-input v-model="form.coupon" placeholder="请输入优惠券总额"></el-input>
        </el-form-item>
        <el-form-item label="状态" v-if="dialogStatus == 'update' && sys_user_del ">
          <el-select class="filter-item" v-model="form.delFlag" placeholder="请选择">
            <el-option v-for="item in statusOptions" :key="item" :label="item | statusFilter" :value="item"> </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="代理商名称" prop="agentName">
          <el-input v-model="form.agentName" placeholder="请输入代理商名称"></el-input>
        </el-form-item>
        <el-form-item label="绑定App">
            <el-checkbox-group 
              v-model="userApps">
              <el-checkbox v-for="(item, index) in appList" :label="item.appId" :key="index" border>{{item.appName}}</el-checkbox>
            </el-checkbox-group>
        </el-form-item>
      </el-form>
...
import { fetchAppIdList, bindApp } from "@/api/appId";
...
export default {
  ...
  methods: {
    ...
    async getAppIdList(){
      const res = await fetchAppIdList();
      this.appList = res.data; 
    },
    async getAppList(){
        let [all, current] = await Promise.all([fetchAppIdList(), bindApp()]);
        if(all.status == 200) localStorage.setItem('allApp', JSON.stringify(all.data));
        if(current.status == 200) {
          this.setAppIdList(current.data);
        }
    },
    getNodeData(data) {
    ...
    handleUpdate(row) {
      this.userApps = row.userApps ? row.userApps : [];
      if(row.userId){
        getObj(row.userId).then(response => {
          let coupon = response.data.coupon;
          this.form = response.data;
          this.form.coupon = Number(coupon)/10000;
          this.dialogFormVisible = true;
          this.dialogStatus = "update";
          this.form.role = [];
          for (var i = 0; i < row.roleList.length; i++) {
            this.form.role[i] = row.roleList[i].roleId;
          }
    ...
    handleChangeTage(val){
      this.form = Object.assign({}, this.form);
    },
    create(formName) {
      const set = this.$refs;
      // this.form.role = this.role;
      this.form.userApps = this.userApps;
